...
contract('SampleCollateralToken', function(accounts) {
    var tokenInstance;
    //var admin = accounts[0];
    //var adminInitialBallance = 100000000; 
    ...
    it('initializes the contract with the correct values', function() {
        ...
        return tokenInstance.balanceOf(accounts[0]);
          ...
          assert.equal(adminBalance, 100000000000000000000, 'admin balance set correctly');
    ...
    //// test works, but must ajust balances to new  mint value  
    //   it('transfers token ownership', function() {
    //     return SampleCollateralToken.deployed().then(function(instance) {
    //       tokenInstance = instance;
    //       // Test `require` statement first by transferring something larger than the sender's balance
    //       return tokenInstance.transfer.call(accounts[1], 5000000, { from: accounts[0] });
    //     }).then(function(success) {
    //       assert.equal(success, true, 'it returns true');
    //       return tokenInstance.transfer(accounts[1], 15000000, { from: accounts[0] });
    ...
    //       assert.equal(balance.toNumber(), 15000000, 'adds the amount to the receiving account');
    //       return tokenInstance.balanceOf(accounts[0]);
    //     }).then(function(balance) {
    //       assert.equal(balance.toNumber(), 85000000, 'deducts the amount from the sending account');
    //     });
    //   });
